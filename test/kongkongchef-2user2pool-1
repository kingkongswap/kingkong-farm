...
describe('KingKongChef-2user-2pool-1', function () {
	...
	before(async function () {
		...
		erc2 = await ERC.deploy('USDT', b(10000), 18);
		await erc2.deployed();
	...
	it('addPool', async function () {
		...
		await erc2.transfer(chef.address, b(10000))
		let blockNum = await ethers.provider.getBlockNumber()
		startBlock = blockNum + 4
		await chef.addPool(erc2.address, b(rewardPerBlock), b(startBlock), b(startBlock+100), accounts[0].address)
		console.log('account0 addPool erc2')
		...
		console.log('account0 addPool erc')
	...
	it('removePool', async function () {
		...
		console.log('mint erc amount:', n(pool.amount), 'balance:', n(pool.balance), 'num:', num)
		pool = await chef.poolMap(erc2.address)
		console.log('mint erc2 amount:', n(pool.amount), 'balance:', n(pool.balance), 'num:', num)
		...
		await chef.removePool(erc2.address)
		console.log('account0 removePool erc2')
		
		assert(n(await kkt.balanceOf(accounts[0].address)) == 20, 'account0 kkt error')
		assert(n(await chef.balanceOf(accounts[0].address)) == 0, 'account0 balance_in_chef error')
		assert(n(await erc2.balanceOf(accounts[0].address)) == 3494, 'account0 erc error')
		assert(n(await kkt.balanceOf(accounts[1].address)) == 20, 'account1 kkt error')
		assert(n(await chef.balanceOf(accounts[1].address)) == 0, 'account1 balance_in_chef error')
		assert(n(await erc2.balanceOf(accounts[1].address)) == 6506, 'account1 erc error')
		assert(n(await kkt.balanceOf(chef.address)) == 0, 'chef kkt error')
	...
	async function print() {
		...
		console.log(
			...
			'erc2:', n(await erc2.balanceOf(accounts[0].address)),
			'pending2:', n(await chef.pendingReward(accounts[0].address, erc2.address)),
			)
		console.log(
			...
			'erc2:', n(await erc2.balanceOf(accounts[1].address)),
			'pending2', n(await chef.pendingReward(accounts[1].address, erc2.address))
			)
		console.log(
