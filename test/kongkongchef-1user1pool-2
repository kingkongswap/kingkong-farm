...
//完整走完一个稍复杂流程
describe('KingKongChef-1user-1pool-2', function () {
	let accounts
	let erc
	let kkt
	let chef
	...
	it('addPool', async function () {
		...
		await erc.transfer(chef.address, b(100))
	...
	it('removePool', async function () {
		...
		assert(n(await erc.balanceOf(accounts[0].address)) == 56, 'account0 erc error')
		assert(n(await kkt.balanceOf(chef.address)) == 0, 'chef kkt error')
		assert(n(await erc.balanceOf(chef.address)) == 44, 'chef erc error')
