...
describe('KingKongChef-proxy', function () {
	let accounts
	let erc
	let kkt
	let deployer
	...
	before(async function () {
		...
		await deployer.createChef(kkt.address, accounts[0].address)
        chefAddress = await deployer.allChefs(0)
        console.log('chef:', chefAddress)

        let chefabi = getAbi('./artifacts/contracts/KingKongChef.sol/KingKongChef.json')
		chef = new ethers.Contract(chefAddress, chefabi, accounts[0])
